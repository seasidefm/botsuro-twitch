...
class DB:
    ...
    async def save_request(self, req: SongRequest):
        ...
        print(
            f"Saved request from {req['user']} -> '{req['artist']} - {req['song_title']}'"
        )
        return result
    ...
    async def __super_saver(self, user: UserPayload, song_to_superfave):
        ...
        already_superfaved = super_faves.find_one(
            {"user": user["user"], "song": song_to_superfave}
        )
        ...
        if len(results) == 0:
            ...
            res = fave_songs.insert_one(
                {
                    "user": user["user"],
                    "twitch_id": user["user_id"],
                    "songs": [{"song": song_to_add, "date": int(time.time())}],
                }
            )
            print(res)
            return "created"
            ...
            if song_to_add not in song_titles:
                new_song_list = fave_data["songs"] + [
                    {"song": song_to_add, "date": int(time.time())}
                ]
                fave_songs.update_one(
                    {"user": {"$eq": user["user"]}},
                    {"$set": {"twitch_id": user["user_id"], "songs": new_song_list}},
                )
                return "added"
            else:
                return "already_exists"
